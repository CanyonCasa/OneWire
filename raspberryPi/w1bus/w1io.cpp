...
/*
  ...
  try {
    this->io.request({ this->consumer, gpiod::line_request::DIRECTION_OUTPUT,
      //gpiod::line_request::FLAG_OPEN_DRAIN | gpiod::line_request::FLAG_BIAS_PULL_UP }, 1);
      gpiod::line_request::FLAG_OPEN_DRAIN }, 1);
  } catch(...) {
      Napi::Error::New(env, "Cannot access GPIO line: "+std::to_string(this->gpio)).ThrowAsJavaScriptException();
      return;
  }
...
Napi::Value W1io::Reset(const Napi::CallbackInfo& info) {
  ...
  // perform a bus reset...
  // set to open drain/pullup to terminate any posible strong drive
  //this->io.set_flags(gpiod::line_request::FLAG_OPEN_DRAIN | gpiod::line_request::FLAG_BIAS_PULL_UP);  
  this->io.set_flags(gpiod::line_request::FLAG_OPEN_DRAIN);  
...
int W1io::bitBang(int bit, bool strong) {
  ...
  if (bit) {
    ...
    usleep(this->t.slot);
    this->io.set_value(1);
    if (strong) this->io.set_flags(0);   // sets output to full push/pull drive
    rd = 0;
  }
  usleep(this->t.rec);
  return rd;
...
int W1io::setValue(unsigned int bit) {
  gpiod_line_set_value(this->line, bit);
}
